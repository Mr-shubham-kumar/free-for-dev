...
def insights_page(resources: list[Resource]) -> str:
    category_counts = Counter(resource.category for resource in resources)
    stop_words = {"about", "and", "are", "for", "free", "from", "into", "its", "that", "the", "this", "with", "your"}
    signals: dict[str, list[Resource]] = defaultdict(list)
    for resource in resources:
        terms = {f"#{tag}" for tag in resource.tags}
        terms.update(
            word.lower()
            for word in re.findall(r"[A-Za-z][A-Za-z0-9+-]{2,}", f"{resource.name} {resource.description}")
            if word.lower() not in stop_words
        )
        for term in terms:
            signals[term].append(resource)

    connections = [
        (tag, linked)
        for tag, linked in signals.items()
        if len({resource.category for resource in linked}) > 1
    ]
    connections.sort(key=lambda item: (-len({resource.category for resource in item[1]}), -len(item[1]), item[0]))
    category_cards = "".join(
        f"<li><strong>{count}</strong> {escaped(category)}</li>"
        for category, count in category_counts.most_common(8)
    ) or "<li>No categories are available yet.</li>"
    connection_cards = "".join(
        f"""<article class="insight-card"><span class="node-label">Shared signal</span><h2>{escaped(tag)}</h2><p>Links {len(linked)} resources across {len({resource.category for resource in linked})} categories.</p><ul class="tags">{"".join(f"<li>{escaped(category)}</li>" for category in sorted({resource.category for resource in linked}))}</ul><p class="connection-links">{" · ".join(f'<a href="resources/{escaped(resource.identifier)}.html">{escaped(resource.name)}</a>' for resource in linked[:3])}</p></article>"""
        for tag, linked in connections[:8]
    ) or "<p class=\"empty-insight\">Add the same hashtag to resources in different categories to reveal cross-category connections.</p>"
    return f"""<!doctype html>
<html lang="en">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="color-scheme" content="light dark"><title>Insights · Research Directory</title><link rel="stylesheet" href="assets/site.css"></head>
<body><main><header class="hero insights-hero"><div class="hero-top"><span class="eyebrow">Signal explorer</span><a class="theme-toggle" href="index.html">Back to directory</a></div><h1>Unexpected <em>connections.</em></h1><p>These are evidence-backed bridges: shared tags that occur across separate research categories. They suggest places worth exploring, not claims of a provider relationship.</p></header><section class="insights-layout"><article class="insight-summary"><span class="node-label">Category map</span><h2>Where the research clusters</h2><ul class="category-list">{category_cards}</ul></article><section class="connections" aria-label="Unexpected connections"><div class="section-heading"><span class="node-label">Cross-category signals</span><h2>Unexpected connections</h2></div><div class="resource-grid">{connection_cards}</div></section></section><footer><a href="index.html">Back to directory</a><span>Connections are derived from source tags and shared vocabulary</span></footer></main></body></html>"""
